...
public class ClientNode implements Runnable
{
		...
		{    
			...
			try
			{
				...
				{	        	  
					...
					int hopCount = Integer.parseInt(st[5]) + 1;
	...
	private InetAddress getIPFromString(String IP)
	{
		...
		int ilast = Integer.parseInt(splitIP[3]);
		byte bLast = (byte)ilast;
		int bLast2 = bLast & 0xFF;
		
		try {
			InetAddress myIp = InetAddress.getByName(IP);
			String str  = myIp.getHostAddress();
		} catch (UnknownHostException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		
		byte[] ipAddr = new byte[]{Byte.parseByte(splitIP[0]), Byte.parseByte(splitIP[1]), Byte.parseByte(splitIP[2]), bLast};
	 ...
	 */
	  ...
	  public void fileSearchOption(String fileName)
	  {
		  ...
		  {
			  System.out.println(alMatchingFileNameList.size()+" File found.");
			  for(int i=0; i<alMatchingFileNameList.size(); i++)
			  {
